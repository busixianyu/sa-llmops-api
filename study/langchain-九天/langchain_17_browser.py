...
# model = init_chat_model(model="qwen3:0.6b", model_provider="ollama")
@tool
def summarize_website(url: str) -> str:
    ...
    try:
        ...
        summary_model = init_chat_model(model="qwen3:0.6b", model_provider="ollama")
        # model = ChatTongyi(model="qwen-plus-latest", api_key=os.getenv("DASHSCOPE_API_KEY"), verbose=True)
        agent = create_openai_tools_agent(summary_model, tools, prompt)
        agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True)
        # 定义任务
        command = {
            "input": "访问这个网站 {url} 并帮我总结一下这个网站的内容"
        }
        # 执行任务
        response = agent_executor.invoke(command)
        return response.get("output", "无法获取网站内容")
...
tools = [summarize_website, generate_pdf]
model = ChatTongyi(model="qwen-plus-latest", api_key=os.getenv("DASHSCOPE_API_KEY"), verbose=True)
